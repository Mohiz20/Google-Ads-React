...
const VideoAdPlayer = () => {
  ...
  const requestAdsFromContainer = (container, adTagIndex = 0) => {
    try {
      setLoadingStatus('Requesting ads...');
      setAdError(null);
      setAdCompleted(false);
      
      // Multiple ad tags to fallback to if one fails
      const adTags = [
  ...
  const loadNewAds = () => {
    if (adDisplayContainer) {
      requestAdsFromContainer(adDisplayContainer, 0); // Start with first ad tag
    } else {
    ...
    <div style={{ maxWidth: '640px', margin: '0 auto' }}>
      ...
      <div style={{ marginTop: '10px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
        <button 
          ...
          style={{
            padding: '8px 16px',
            backgroundColor: adsLoaded ? '#4CAF50' : '#cccccc',
            color: 'white',
            border: 'none',
            borderRadius: '4px',
            cursor: adsLoaded ? 'pointer' : 'not-allowed'
          }}
        >
          {adsLoaded ? 'Play Ads' : 'Loading...'}
        </button>

        {/* Show "Load New Ads" button after ad completes or if no ads loaded */}
        {(adCompleted || (!adsLoaded && !loadingStatus.includes('Checking'))) && (
          <>
            <button 
              onClick={loadNewAds}
            ...
            {/* Alternative ad sources buttons */}
            <button 
              onClick={() => adDisplayContainer && requestAdsFromContainer(adDisplayContainer, 1)}
              disabled={!adDisplayContainer}
              style={{
                padding: '8px 16px',
                backgroundColor: adDisplayContainer ? '#FF9800' : '#cccccc',
                color: 'white',
                border: 'none',
                borderRadius: '4px',
                cursor: adDisplayContainer ? 'pointer' : 'not-allowed',
        ...
        <p style={{ fontSize: '12px', color: '#666', marginTop: '10px', width: '100%' }}>
